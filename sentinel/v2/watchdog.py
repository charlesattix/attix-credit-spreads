...
def _get_active_experiments() -> list[dict]:
    ...
    if not base or not _DASHBOARD_API_KEY:
        LOG.warning("RAILWAY_SERVICE_ATTIX_DASHBOARD_URL or DASHBOARD_API_KEY not set")
        return []
    try:
        resp = _requests.get(
...
def main() -> None:
    ...
    if not _DASHBOARD_URL:
        LOG.warning("RAILWAY_SERVICE_ATTIX_DASHBOARD_URL not set — sentinel cannot reach dashboard")
